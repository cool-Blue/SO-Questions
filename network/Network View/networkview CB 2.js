...
function tick(e) {
	...
	if (call_links_data.length > 0) {
		...
		.each(function (d) {
			d.lps1 = line_perpendicular_shift(d, 1)
			d.lrste = []
		...
		//CB
		.attr("x1", function (d) {
			return d.source.x - d.lps1[0] + d.lrste[0][0];
		})
		.attr("y1", function (d) {
			return d.source.y - d.lps1[1] + d.lrste[0][1];
		})
		.attr("x2", function (d) {
			return d.target.x - d.lps1[0] + d.lrste[1][0];
		})
		.attr("y2", function (d) {
			return d.target.y - d.lps1[1] + d.lrste[1][1];
		});
 ...
 */
	...
	var lineLengthCalculation = function (x, y, x0, y0) {
