window.onload = function () {
	...
	var sampleSVG = d3.select("#viz")
			...
			.attr("width", 1000)
			.attr("height", 200)
			//.on('mouseover.viz', function () {
			//	if (d3.event.target.tagName != 'svg') {
			//		console.log('viz\t\t' + d3.event.type + '\t' + d3.select(event.target).data())
			//	}
			//})
			.on("mouseover", (function () {
				var prevNode, interlock = '__onmouseout';
				return function () {
					var target = d3.event.target, d = target.__data__;
					if (target.tagName === 'circle') {
						if (prevNode && !(prevNode === target)
								&& prevNode.___active___) {
							console.log('***collision***\tmouseover\t' + d)
							prevNode[interlock] && prevNode[interlock]()
						}

						prevNode = target
						prevNode.___active___ = true

						if (!activeTransition(target).length) {
			...
			//.on("mouseout", function () {
			//  var target = d3.event.target || this, d = target.__data__;
			//	console.log('^mouseout\t\t' + target.tagName)
			//  if (target.tagName === 'circle') {
			//	  console.log('mouseout\t\t' + d);
			//	  if (!activeTransition(target).length) {
			//		  if (target) {
			//			  target.___active___ = false
			//		  }
			//		  d3.select(target)
			//			  .style("fill", "white")
			//			  .attr("r", 40);
	...
	function listener(e) {
		...
		console.log('svg-test ' + e.type + ', target: ' + source.id)
