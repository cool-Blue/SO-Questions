window.onload = function () {
	...
	circles.on("mouseover", (function () {
				var prevNode, interlock
				return function (d) {

					if (prevNode && !(prevNode === this) && //prevNode.__onmouseover &&
									(interlock = prevNode.__onmouseover.___interlock___) &&
									interlock.___active___) {
			...
			.on("mouseout", function (d) {
				...
				if (!activeTransition(this).length) {
					var interlock = this.__onmouseover
					if (interlock) {
						interlock.___interlock___ = arguments.callee.bind(this)
						interlock.___interlock___.___active___ = false
					} else {
						console.log('******mouseOut active transition******')
					}
					d3.select(this)
						.style("fill", "white")
						.attr("r", 40);
	...
	function listener(e) {
		...
		console.log('svg-test mouseover, target: ' + source.id)
