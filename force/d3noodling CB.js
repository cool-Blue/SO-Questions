...
force = d3.layout.force()
	...
	//Link bag
			...
			paths = svg.selectAll(".paths")
				.data(["paths_g"]),
			//ENTER
			pathsEnter = paths.enter()
				.append("svg:g")
				.attr("class", "paths"),

	//Links
			//UPDATE
			pathUpdate = paths.selectAll("path")
				.data(links),
			//ENTER
	...
	//EXIT			
	pathExit = pathUpdate.exit().remove();

	//Link labels bag
			//UPDATE
	var path_labelsUpdate = svg.selectAll(".labels")
			.data(["labels_g"]),
			//ENTER
			path_labelsEnter = path_labelsUpdate.enter()
				.append("svg:g")
				.attr("class", "labels"),

	//Link labels
			//UPDATE
			pathLabelUpdate = path_labelsUpdate.selectAll(".path_label")
				.data(links),
			//ENTER
			pathLabelEnter = pathLabelUpdate.enter()
				.append("svg:text")
					.append("svg:textPath")
						.attr("startOffset", "50%")
			...
			//UPDATE+ENTER
			path_label = pathLabelUpdate
				.attr("class", function (d, i) { return "path_label " + i })
				.attr("xlink:href", function (d) { return "#" + d.source.index + "_" + d.target.index; })
				.text(function (d) { return d.type; }),
			//EXIT
			pathLabelExit = pathLabelUpdate.exit().remove(),

	//Nodes bag
			//UPDATE
			circlesUpdate = svg.selectAll(".circles")
				.data(["circles_g"]),
			//ENTER
			circlesEnter = circlesUpdate.enter()
				.append("svg:g")
				.attr("class", "circles"),

	//Nodes
			//UPDATE
			circleUpdate = circlesUpdate.selectAll(".nodes")
				.data(nodes),
			//ENTER
			circleEnter = circleUpdate.enter().append("svg:circle")
